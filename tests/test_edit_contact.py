...
def test_edit_rand_contact_from_homepage(app):
    if app.contact.count() == 0:
        app.contact.create(Contact(firstname="test"))
    contact = Contact(firstname="rand", middlename="Jasd", lastname="homepage", nickname="Madsti",
                               title="adasor", company="Hoasdadood", address="Ladngeles")
    old_contacts = app.contact.get_contact_list()
...
def test_edit_rand_contact_from_details(app):
    if app.contact.count() == 0:
       app.contact.create(Contact(firstname="test"))
    contact = Contact(firstname="rand", middlename="Jasd", lastname="editing", nickname="Madsti",
                               title="adasor", company="Hoasdadood", address="Ladngeles")
    old_contacts = app.contact.get_contact_list()
    index = randrange(len(old_contacts))
    contact.id = old_contacts[index].id
